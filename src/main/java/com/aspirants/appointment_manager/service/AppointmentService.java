...
public class AppointmentService {
        ...
        public AppointmentResponse createAppointment(AppointmentRequest request) {
                ...
                // Validate store matches service
                if (!service.getStore().getStoreId().equals(store.getStoreId())) {
                        throw new InvalidRequestException("Service does not belong to the specified store");
                }
                ...
                List<Appointment> conflicts = appointmentRepository.findConflictingAppointments(
                                store, request.getAppointmentDate(), request.getStartTime(), endTime);
        ...
        @Transactional(readOnly = true)
        public List<AppointmentResponse> getAppointmentsByStore(Long storeId) {
                Store store = storeRepository.findById(storeId)
                                .orElseThrow(() -> new ResourceNotFoundException("Store", "id", storeId));

                return appointmentRepository.findByStore(store).stream()
                                .map(this::mapToResponse)
                                .collect(Collectors.toList());
        }
